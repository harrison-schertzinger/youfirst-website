...
import { type TryoutDisplay } from "@/lib/tryouts";
...
export async function sendTryoutConfirmationEmail({
  ...
  const subject = `You're registered: ${playerFullName}'s YOU. FIRST ${
    display.type === "evaluation" ? "evaluation" : "tryout"
  }, ${display.dateLine}`;
...
function buildText({
  ...
  const isEval = display.type === "evaluation";
  return [
    `Hi ${parentName},`,
    ``,
    `We are glad to have her. ${playerFullName} is officially registered for YOU. FIRST 2026 ${isEval ? "evaluations" : "tryouts"} — completely free. Her spot is secured.`,
    ``,
    `${isEval ? "EVALUATION" : "TRYOUT"} (${display.typeLabel}): ${display.dateLine}`,
    `TIME: ${display.time}`,
    ...
    ``,
    isEval
      ? `If anything comes up before then, just reply to this email and it will reach us.`
            ...
            <tr>
              <td style="padding:32px;">
                ...
                <p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#374151;">
                  ...
                  registered for YOU. FIRST 2026 ${isEval ? "evaluations" : "tryouts"} —
                  <strong>completely free</strong>. Her spot is secured.
                </p>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#EDF5FB;border:1px solid rgba(75,156,211,0.25);border-radius:12px;margin-bottom:24px;">
                  <tr>
                    <td style="padding:20px 24px;">
                      <div style="font-size:11px;letter-spacing:0.15em;text-transform:uppercase;color:#4B9CD3;font-weight:600;">${isEval ? "Free Evaluation" : `${escapeHtml(display.typeLabel)} Tryout`}</div>
